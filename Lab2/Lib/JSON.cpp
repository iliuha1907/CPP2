...
void JSON::add(const string& key, Value value) {
...
void JSON::edit(const std::string& forKey, const Value& newValue) {
    if (holds_alternative<KeyValue>(root)) {
        auto v_map = get<KeyValue>(root);
...
void JSON::edit(int atIndex, const Value& newValue) {
    if (holds_alternative<Array>(root)) {
        auto v_array = get<Array>(root);
...
void JSON::remove(const std::string& forKey) {
    if (holds_alternative<KeyValue>(root)) {
        auto v_map = get<KeyValue>(root);
        if (v_map.find(forKey) != v_map.end()) {
